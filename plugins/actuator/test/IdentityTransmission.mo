...
equation
  connect(invertSign.y, jointTorque) annotation(
    Line(points = {{73, 16}, {80, 16}, {80, 60}, {110, 60}}, color = {0, 0, 127}));
  connect(angleToTorqueAdaptor1.tau, invertSign.u) annotation(
    Line(points = {{34, 16}, {50, 16}}, color = {0, 0, 127}));
  connect(torque1.flange, angleToTorqueAdaptor1.flange) annotation(
    Line(points = {{-30, 0}, {23, 0}}));
  connect(jointVelocity, angleToTorqueAdaptor1.w) annotation(
    Line(points = {{118, 0}, {79.5, 0}, {79.5, -6}, {35, -6}}, color = {0, 0, 127}));
  connect(jointPosition, angleToTorqueAdaptor1.phi) annotation(
    Line(points = {{120, -60}, {80.5, -60}, {80.5, -16}, {35, -16}}, color = {0, 0, 127}));
  connect(actuatorInput, torque1.tau) annotation(
    Line(points = {{-120, 0}, {-52, 0}, {-52, 0}, {-52, 0}}, color = {0, 0, 127}));
  annotation(
    uses(Modelica(version = "3.2.2")));
end IdentityTransmission;
