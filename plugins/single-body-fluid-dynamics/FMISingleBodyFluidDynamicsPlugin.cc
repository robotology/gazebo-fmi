...
#else
    physicsEngineName = gazebo::physics::get_world()->GetPhysicsEngine()->GetType();
#endif

    if (physicsEngineName == "bullet")
    {
        gzerr << "FMISingleBodyFluidDynamicsPlugin: the bullet physics engine is not supported, since using Bullet in Gazebo there is no way to excert an external force on a link." << std::endl;
...
#else
    ignition::math::Vector3d worldRelativeVel = link->GetWorldLinearVel().Ign()-link->WorldWindLinearVel();
#endif
